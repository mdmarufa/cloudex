...
// --- Premium Control Button Component (Preserved) ---
...
const styles = `
  .slide-enter-right { animation: slideInRight 0.4s cubic-bezier(0.2, 0.8, 0.2, 1) forwards; }
  .slide-exit-left   { animation: slideOutLeft 0.4s cubic-bezier(0.2, 0.8, 0.2, 1) forwards; }
  
  .slide-enter-left  { animation: slideInLeft 0.4s cubic-bezier(0.2, 0.8, 0.2, 1) forwards; }
  .slide-exit-right  { animation: slideOutRight 0.4s cubic-bezier(0.2, 0.8, 0.2, 1) forwards; }
...
// --- Slide Component ---
// Manages individual image loading state and positioning
...
const ImageSlide: React.FC<SlideProps> = ({ 
  file, animClass, zIndex, scale = 1, rotation = 0, position = {x:0, y:0}, isDragging = false 
}) => {
  const [isLoaded, setIsLoaded] = useState(false);
  const [hasError, setHasError] = useState(false);
  
  // Reset loading state if file changes (though usually keys handle this)
  useEffect(() => {
    setIsLoaded(false);
    setHasError(false);
  }, [file.id]);
  ...
  return (
    <div 
        ...
        style={{ zIndex }}
    >
        {/* Loading / Error State (Visible immediately during slide) */}
        {!isLoaded && !hasError && (
            <div className="absolute inset-0 flex items-center justify-center">
                 <div className="flex flex-col items-center gap-3">
                    <Loader2 size={48} className="text-blue-500 animate-spin drop-shadow-lg" />
        ...
        {hasError && (
            <div className="flex flex-col items-center gap-2 p-6 bg-red-500/10 rounded-2xl backdrop-blur-md border border-red-500/20">
                <AlertCircle className="text-red-500" size={32} />
                <span className="text-white font-medium">Failed to load</span>
            </div>
        )}
        ...
        <img 
            ...
            className="max-w-[95%] max-h-[90vh] object-contain shadow-2xl will-change-transform pointer-events-auto"
            style={{
                transform: `translate3d(${position.x}px, ${position.y}px, 0) rotate(${rotation}deg) scale(${scale})`,
                opacity: isLoaded ? 1 : 0,
                transition: isDragging ? 'none' : 'opacity 0.2s ease-out, transform 0.1s linear',
...
const ImageViewer: React.FC<ImageViewerProps> = ({ file, onClose, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    if (file.id !== activeFile.id) {
        ...
        // Remove exiting slide after animation completes (400ms matches CSS)
  ...
  const handleFitWidth = () => {
    // Simplification for demo: just set scale 1, real implementation requires measuring image
    setScale(1); setPosition({ x: 0, y: 0 });
  };
  const handleFitHeight = () => {
    setScale(0.8); setPosition({ x: 0, y: 0 });
  };
  ...
  // --- Interaction (Pan/Wheel/Touch) ---
  const handleWheel = (e: React.WheelEvent) => {
      e.stopPropagation();
      if (e.ctrlKey || Math.abs(e.deltaY) > 0) {
        if (e.deltaY < 0) setScale(prev => Math.min(prev * 1.1, 10));
        else setScale(prev => Math.max(prev / 1.1, 0.1));
      }
  };
  ...
  // --- Render Helpers ---
  const getAnimationClass = (mode: 'enter' | 'exit') => {
      if (!isAnimating) return '';
      if (direction === 'next') {
          return mode === 'enter' ? 'slide-enter-right' : 'slide-exit-left';
      } else {
          return mode === 'enter' ? 'slide-enter-left' : 'slide-exit-right';
      }
  };
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 inset-x-0 z-50 flex items-center justify-between p-4 sm:p-6 bg-gradient-to-b from-black/80 via-black/40 to-transparent pointer-events-none transition-opacity duration-300 hover:opacity-100">
        <div className="flex items-center gap-3 sm:gap-4 pointer-events-auto flex-1 min-w-0 pr-4">
             ...
             <div className="flex flex-col text-white drop-shadow-md min-w-0">
                <h3 className="text-sm sm:text-base font-bold truncate leading-tight w-full">{activeFile.name}</h3>
        ...
        <div className="flex items-center gap-2 pointer-events-auto flex-shrink-0">
             <div className="hidden md:flex items-center gap-2">
                <ControlBtn icon={ExternalLink} label="Open Original" onClick={() => {}} tooltipPlacement="bottom" />
             ...
             <div className="md:hidden relative" ref={menuRef}>
                 ...
                 {isMobileMenuOpen && (
                     <div className="absolute right-0 top-full mt-2 w-48 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl overflow-hidden py-1 z-50 animate-in fade-in zoom-in-95 origin-top-right">
                         <button className="w-full text-left px-4 py-3 text-sm text-slate-200 hover:bg-white/10 flex items-center gap-3"><ExternalLink size={16} /> Open Original</button>
                         <button className="w-full text-left px-4 py-3 text-sm text-slate-200 hover:bg-white/10 flex items-center gap-3"><Download size={16} /> Download</button>
                     </div>
                 )}
             </div>
      ...
      {/* Main Viewport */}
      <div ref={containerRef} className="flex-1 relative w-full h-full z-10 overflow-hidden" onClick={(e) => e.stopPropagation()}>
         ...
         {exitingFile && (
             <ImageSlide 
                key={exitingFile.id}
                file={exitingFile}
                animClass={getAnimationClass('exit')}
                zIndex={0}
             />
         )}
         ...
         <ImageSlide 
            key={activeFile.id}
            file={activeFile}
            animClass={getAnimationClass('enter')}
            zIndex={1}
            scale={scale}
            rotation={rotation}
            position={position}
      ...
      <div className="absolute bottom-0 inset-x-0 z-50 flex flex-col items-center pb-8 pt-24 bg-gradient-to-t from-black/90 via-black/50 to-transparent pointer-events-none">
         ...
         {/* Filmstrip */}
         {allImages.length > 1 && (
            <div ref={filmstripRef} className="relative flex items-center gap-3 p-3 mb-6 bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl overflow-x-auto w-auto max-w-[calc(100%-2rem)] sm:max-w-2xl pointer-events-auto custom-scrollbar scroll-smooth mx-4">
                {allImages.map((img) => (
                    <div 
                        key={img.id}
         ...
         <div className="pointer-events-auto w-full px-4 flex justify-center">
             <div className="flex items-center gap-1 sm:gap-1.5 p-1.5 sm:p-2 bg-slate-900/80 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] ring-1 ring-white/5 overflow-x-auto max-w-full">
